"""
Copyright (C) 2019 by Maria Antoniak and Melanie Walsh: https://github.com/maria-antoniak/goodreads-scraper
License: https://github.com/maria-antoniak/goodreads-scraper/blob/master/LICENSE

This script retrieves goodread reviews for given list of books. Some minor modifications have been made from the original code licnesed above to suit the need of this project:
- added pop-up close action when encountering 
- updated the ElementClickInterceptedException handling logic to close the pop-up window and try scraping the page again, rather than skipping it
- removed chrome from browser option
- added setup_driver function to easily test for both headless and non-headless browsers
"""
...
def get_reviews_first_ten_pages(driver, book_id, sort_order):
    ...
    try:
        ...
        while page_counter <=10:
            try:
                if len(driver.find_elements(By.LINK_TEXT, str(page_counter))) > 0:
                    elements = driver.find_elements(By.LINK_TEXT, str(page_counter))
                    elements[-1].click()
                    time.sleep(4)
                    reviews += scrape_reviews_on_current_page(driver, url, book_id, sort_order)
                ...
                print(f'🚨 ElementClickInterceptedException (Likely a pop-up)🚨\n🔄 Attempting to remove popup and try page again {page_counter}🔄')
                driver.find_element(By.CLASS_NAME, 'Icon CloseIcon').click()
                reviews += scrape_reviews_on_current_page(driver, url, book_id, sort_order)
                print(f"Scraped page {page_counter}")
                page_counter += 1
                continue
